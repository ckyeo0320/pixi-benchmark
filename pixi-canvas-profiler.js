...
(function() {
  ...
  (function patchCanvas2D() {
    ...
    ["fill", "stroke", "fillRect", "strokeRect"].forEach(fn => wrap(fn, "path"));
  ...
  function __getAtlasIdFromBaseTexture(baseTexture) {
    ...
    // 기존 소스가 있으면 우선 사용
    const src = baseTexture.source || baseTexture.imageUrl || baseTexture._imageUrl;
    if (src) {
      id = src;
      // 이미지 URL에서 파일명 추출
  ...
  function applyPixiPatches() {
    ...
    if (PIXI.spine && PIXI.spine.SpineBase && PIXI.spine.SpineBase.prototype._renderCanvas) {
          ...
          try {
            ...
            if (this.skeleton && this.skeleton.slots) {
              for (let i = 0; i < this.skeleton.slots.length; i++) {
                ...
                if (attachment && attachment.texture && attachment.texture.baseTexture) {
